...
pub fn get_meta_inf_entry_names(frame: &mut Frame) {
    ...
    let zip_file = crate::native::java::util::zip_file::zip_file_cache::get_mut(address).expect("the file is not open");
    let mut data = Vec::new();
    if zip_file.file.by_name("META-INF/").is_ok() {
        data.push(Some(StringPool::java_string("META-INF/".to_string())));
    }
    if zip_file.file.by_name("META-INF/MANIFEST.MF").is_ok() {
        data.push(Some(StringPool::java_string("META-INF/MANIFEST.MF".to_string())));
    }
    let boot = Jvm::boot_class_loader();
    let object = ArrayObject::from_data(boot.find_or_create("java/lang/String"),DataType::References(data));
    frame
        .operand_stack()
