...
struct EnclosingMethodAttribute<'a> {
    cp:&'a ConstantPool,
    class_index:u16,
    method_index:u16
}

impl EnclosingMethodAttribute<'_> {
    pub fn class_name(&self) -> &str {
        return get_utf8(self.cp,self.class_index as usize);
    }

    pub fn method_name_and_descriptor(&self) -> (&str,&str) {
        if self.class_index > 0 {
            return get_name_and_type(self.cp,self.method_index as usize);
        }
        return ("","")
    }
}

impl AttributeInfo for EnclosingMethodAttribute<'_> {
